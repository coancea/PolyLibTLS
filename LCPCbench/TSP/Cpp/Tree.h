...
private:
  ...
  static const double  M_E2  = 7.3890560989306502274;
  static const double  M_E3  = 20.08553692318766774179;
  static const double  M_E6  = 403.42879349273512264299;
  static const double  M_E12 = 162754.79141900392083592475;
...
//    cout<<"In conquer TLS: "<<" "<<(unsigned long)(void*)t<<endl;
  ...
  static double median(double min, double max, int n) 
  {
    ...
    double t = nextDouble();        // MODIFY RANDOM!!!
   ...
   **/
  static Tree* buildTree(int n, bool dir, double min_x,
...
private:
  void fillIteratorHelp(int dim, int ind1, int ind2);
  void fillIterator    (int sz);
  

public:

  static Tree***  array_iter;
  static int      array_dim;
   ...
   **/
  Tree* tsp_orig(int sz);
  Tree* tsp     (int sz);
  Tree* tsp_TLS (int sz);

//  template<class TH>
//  Tree* conquer_TLS (TH* th);

  void tsp_conq_TLS (int sz);
  Tree* conquer_TLS (Thread_TSP_Conq_TLS* th);
  Tree* makeList_TLS(Thread_TSP_Conq_TLS* th);
  
  void tsp_merge_TLS(int sz);
  Tree* merge_TLS   (Tree* left, Tree* right, Thread_TSP_Merge_TLS* th);
  void  reverse_TLS (Thread_TSP_Merge_TLS* th);
};
...
public:
  TreeClos() { ptr = NULL; }
  TreeClos(Tree** p)  { ptr = p; }

  Tree* get()        { return *ptr; }
  void  set(Tree* t) { *ptr = t;    }
  bool  empty()      { return ptr == NULL; }

  Tree* get_TLS(Thread_TSP_Merge_TLS* th);
  Tree* get_TLS(Thread_TSP_Conq_TLS*  th);

  void  set_TLS(Tree* t, Thread_TSP_Merge_TLS* th);
  void  set_TLS(Tree* t, Thread_TSP_Conq_TLS*  th);
};
...
public:
  ...
  bool     hasMoreElements_TLS(Thread_TSP_Merge_TLS* th);
  bool     hasMoreElements_TLS(Thread_TSP_Conq_TLS*  th);

  TreeClos nextElement_TLS    (Thread_TSP_Merge_TLS* th);
  TreeClos nextElement_TLS    (Thread_TSP_Conq_TLS*  th);
};
